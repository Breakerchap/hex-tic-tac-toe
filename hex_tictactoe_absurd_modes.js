...
function makeInitialState(modeKeys) {
...
function getBirdAt(state, hex) {
  const entry = getBirdEntries(state).find(({ hex: birdHex }) => equalHex(birdHex, hex));
  return entry ? entry.birdKind : null;
}

function isStoneOccupied(state, hex) {
  return Boolean(state.cells[keyOf(hex.q, hex.r)]);
}
...
function isLegalByBaseRules(state, hex) {
  if (state.winner) {
    return false;
  }
  if (isOccupied(state, hex)) {
    return false;
  }
...
function isLegalPlacement(state, hex) {
  return isLegalByBaseRules(state, hex);
}
...
function countsForOwnerAt(state, pos, owner) {
  const cell = state.cells[keyOf(pos.q, pos.r)];
  return Boolean(cell && cell.owner === owner && cell.kind === "stone");
}
...
function checkWinnerFrom(state, hex) {
  const cell = state.cells[keyOf(hex.q, hex.r)];
  if (!cell || cell.kind !== "stone") {
    return 0;
  }
  for (const dir of lineAxes) {
    if (getLineCount(state, hex, cell.owner, dir) >= WIN_LENGTH) {
      return cell.owner;
    }
  }
  return 0;
...
function placeNormalTile(state, hex, owner) {
...
function clickPlacement(hex) {
  ...
  saveHistory();
  placeNormalTile(state, hex, state.turnPlayer);
  pushLog(`Player ${state.turnPlayer} placed at (${state.lastPlacement.q}, ${state.lastPlacement.r}).`);
...
function updateStatus() {
  const state = game.state;
  const mode = getModeConfig(state.modeKeys);
...
function drawGrid() {
  ...
  const radius = Math.ceil(Math.max(w, h) / size) + 6;
  const centerWorld = screenToWorld(w / 2, h / 2);
  const centerHex = pixelToAxial(centerWorld.x, centerWorld.y, size);

  for (let r = -radius; r <= radius; r += 1) {
    for (let q = -radius; q <= radius; q += 1) {
      const hex = { q: centerHex.q + q, r: centerHex.r + r };
      if (hexDistance(hex, centerHex) > radius + 1) {
        continue;
      }
...
function drawPieces() {
  ...
  const w = canvas.clientWidth;
  const h = canvas.clientHeight;
  const recentSerials = Object.values(game.state.cells)
...
function drawWinnerLineHint() {
  ...
  const lastCell = game.state.cells[keyOf(last.q, last.r)];
  if (!lastCell) {
    return;
  }

  for (const dir of lineAxes) {
    if (getLineCount(game.state, last, lastCell.owner, dir) >= WIN_LENGTH) {
      let minStep = 0;
      let maxStep = 0;
      while (true) {
        const pos = { q: last.q + dir.q * (minStep - 1), r: last.r + dir.r * (minStep - 1) };
        if (!countsForOwnerAt(game.state, pos, lastCell.owner)) {
          break;
        }
        minStep -= 1;
      }
...
function newGame(modeKeys = getSelectedModeKeys()) {
  game.state = makeInitialState(modeKeys);
  game.history = [];
  centreBoard();
  updateStatus();
  render();
...
canvas.addEventListener("mousemove", (event) => {
  ...
  game.hoverHex = pixelToAxial(world.x, world.y, currentHexSize());
  render();
});
